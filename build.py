...
async def find_site_packages() -> typing.Coroutine | "Path":
    ...
    raise MemrixError(f"[!] Site packages path not found in virtual environment")
...
async def packaging() -> tuple[
    ...
    launch = app.parent.joinpath(const.SCHEMATIC, "resources", "automation")

    compile_cmd = [exe := sys.executable, "-m", "nuitka", "--standalone"]

    if (ops := sys.platform) == "win32":
        target = app.joinpath(f"{const.APP_DESC}.dist")
        rename = target, app.joinpath(f"{const.APP_DESC}Engine")

        compile_cmd += [
            f"--windows-icon-from-ico=schematic/resources/icons/memrix_icn_1.ico",
        ]

        launch = launch.joinpath(f"{const.APP_NAME}.bat"), target.parent

    elif ops == "darwin":
        target = app.joinpath(f"{const.APP_DESC}.app", f"Contents", f"MacOS")
        rename = target.parent.parent, app.joinpath(f"{const.APP_DESC}.app")

        compile_cmd += [
            f"--macos-create-app-bundle",
            f"--macos-app-name={const.APP_DESC}",
            f"--macos-app-version={const.APP_VERSION}",
            f"--macos-app-icon=schematic/resources/images/macos/memrix_macos_icn.png",
        ]

        launch = launch.joinpath(f"{const.APP_NAME}.sh"), target
...
async def post_build() -> typing.Coroutine | None:
    ...
    async def forward_dependencies() -> typing.Coroutine | None:
        ...
        with (Progress(
                ...
                BarColumn(
                    bar_width=int(Grapher.console.width * 0.3), style="bold #ADD8E6",
    ...
    schematic = app.parent.joinpath(const.SCHEMATIC)
