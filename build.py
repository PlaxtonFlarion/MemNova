...
async def packaging() -> tuple[
    ...
    if (ops := sys.platform) == "win32":
        if (lib_path := venv_base_path / "Lib" / "site-packages").exists():
            ...
            rename = app.joinpath(f"{const.APP_DESC}Engine")
        ...
        if (lib_path := venv_base_path / "lib").exists():
            for sub in lib_path.iterdir():
                ...
                rename = app.joinpath(f"{const.APP_DESC}.app")
...
async def post_build() -> typing.Coroutine | None:
    ...
    schematic = apps.parent.joinpath(const.SCHEMATIC)
    ...
    local_file = [
        (wake.joinpath(f"{const.APP_NAME}.bat"), target.parent)
        if ops == "win32" else (wake.joinpath(f"{const.APP_NAME}.sh"), target)
    ]
